          ...
          {#each types as { definition, number } (definition.key)}
            <li>
              <a
                ...
                onclick={(event) => intercept(event, () => route.visit(number))}
              >
                {number}
              </a>
              <a
                class="what"
                href={pathFor.format(definition.key)}
                onclick={(event) => intercept(event, () => route.show(definition.key))}
              >
                {definition.name}
              </a>
            </li>
